...
#sites =['neva11']
folder_meas = r'C:/Users/alauren/OneDrive - University of Eastern Finland/Stenberg/puusto_ym_data_Arille/Pohjavesiaineistot/'
for crd, k, tx in zip(coordinates, sites, txt):
    print (crd, k) 
    ...
    axs[crd].fill_between(dfsim.index, wt_max, wt_min, color='grey', alpha = 0.3)
    axs[crd].set_ylim([-1.0,0.0])
    axs[crd].plot(dfmeas, 'bo', markersize=2)
    axs[crd].set_xlim([start_date, end_date])
    axs[crd].xaxis.set_major_locator(mdates.YearLocator())
    axs[crd].xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
    
    axs[crd].text(0.02, 0.95, k, horizontalalignment='left',
               verticalalignment='top', fontsize=14, transform = axs[crd].transAxes)

    for xlabel_i in axs[crd].get_xticklabels():
        xlabel_i.set_fontsize(10.0)
        xlabel_i.set_y(0.1)

        
    if crd[1]==1:
        for ylabel_i in axs[crd].get_yticklabels():
            ylabel_i.set_fontsize(0.0)
            ylabel_i.set_visible(False)
    else:
        for ylabel_i in axs[crd].get_yticklabels():
            ylabel_i.set_fontsize(10.0)
            ylabel_i.set_x(-0.025)
            
        axs[crd].set_ylabel('WT, m', fontsize = 14)


    if crd[0]==rows-1:
        axs[crd].set_xlabel('Time', fontsize = 14)
...
folder_out = r'C:/Users/alauren/Documents/WinPython-64bit-2.7.10.3/Susi_8_3_py37/outputs/'
file_vol = 'vols.xlsx'
...
for c, k in enumerate(sites):
    ...
    plt.plot(obs,pre,'o', markersize=10, label=k, color=colors[c])
...
ax0.text(0.02, 0.95, 'a', horizontalalignment='left',
               verticalalignment='top', fontsize=16, transform = ax0.transAxes,  fontweight='bold')
...
for c, s in enumerate(sites):
    ...
    plt.plot(obs,pre,'o', markersize=10, label=s, color=colors[c])
...
ax1.text(0.02, 0.95, 'b', horizontalalignment='left',
               verticalalignment='top', fontsize=16, transform = ax1.transAxes, fontweight='bold')
...
plt.xlabel('Observed volume growth, $m^{3} ha^{-1} yr^{-1}$ ', fontsize =fs )
plt.ylabel('Predicted volume growth, $m^{3} ha^{-1} yr^{-1}$ ', fontsize =fs)
...
ax2.text(0.02, 0.95, 'c', horizontalalignment='left',
               verticalalignment='top', fontsize=16, transform = ax2.transAxes,  fontweight='bold')
